...
tf.app.flags.DEFINE_string('log-dir', '{cwd}/logs_4.2/'.format(cwd=os.getcwd()),
...
def main(_):
    ...
    adversarial_img_summary = tf.summary.image('Adversarial Images', x_image)
    adversarial_test_img_summary = tf.summary.image('Adversarial Test Images', x_image)
    ...
    validation_summary = tf.summary.merge([loss_summary, accuracy_summary, test_img_summary])
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        adversarial_validation_writer = tf.summary.FileWriter(run_log_dir + "_validation_adversarial", sess.graph)
        adversarial_train_writer = tf.summary.FileWriter(run_log_dir + "_train_adversarial", sess.graph)
